...
function GatewayCard(card: SiteGatewayCard) {
  ...
  const cardText = (
    <>
      ...
      <h2 className="text-xl font-extrabold leading-tight text-white sm:text-2xl md:text-3xl max-md:text-[0.95rem] max-md:leading-snug">
        {title}
      </h2>
      <p className="mt-2 max-w-sm text-sm leading-relaxed text-white/85 md:text-base max-md:mt-0.5 max-md:line-clamp-1 max-md:text-[0.7rem] max-md:leading-snug">
        <span className="md:hidden">{shortSubtitle}</span>
        <span className="hidden md:inline">{subtitle}</span>
      </p>
      <span className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-accent transition group-hover:gap-3 md:mt-5 max-md:mt-1 max-md:text-[0.7rem]">
        כניסה לאזור
        <ArrowLeft size={18} className="max-md:size-3" aria-hidden />
      </span>
    </>
  );
  ...
  const cardContent = (
    <>
      {/* Mobile: image on top (full fit), text strip below */}
      <div className="flex h-full flex-col overflow-hidden md:hidden">
        ...
        <div className="shrink-0 border-t border-white/15 bg-primary px-3 py-2.5">{cardText}</div>
      </div>

      {/* Desktop: cinematic overlay card */}
      <div className="relative hidden min-h-[220px] md:block lg:min-h-[280px]">
        <GatewayCardMedia card={card} />
        <GatewayCardOverlay />
        <div className="relative flex min-h-[220px] flex-col justify-end p-5 lg:min-h-[280px] lg:p-8">
          {cardText}
        </div>
      </div>
    </>
    ...
    >
      <div
        className={cn(
          ...
          "max-md:flex max-md:min-h-0 max-md:flex-1 max-md:flex-col max-md:px-3 max-md:pb-6 max-md:pt-4",
        )}
      >
        ...
        <div
          className={cn(
            ...
            "max-md:mt-3 max-md:min-h-0 max-md:flex-1 max-md:grid-rows-3 max-md:gap-3",
          )}
          role="list"
        ...
        <div className="mt-8 shrink-0 text-center sm:mt-10 max-md:mt-3 max-md:pb-1">
          <Link
            to="/about"
