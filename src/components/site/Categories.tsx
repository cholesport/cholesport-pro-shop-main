  ...
  return (
    <section id="categories" className="max-w-7xl mx-auto px-4 py-16 md:py-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-border border border-border">
        {CATEGORIES.map(({ slug, name, icon, image, imageDisplay }, index) => (
          <FadeIn key={slug} preset="card" index={index}>
            <Link
              to="/categories/$categorySlug"
              params={{ categorySlug: slug }}
              className="group flex items-center gap-4 bg-background px-5 py-6 hover:bg-secondary/70 transition h-full"
            >
