...
export function getLandingMatSeoDescription(v: LandingMatVariant) {
  if (v.id === LANDING_MAT_AIRFLOOR_ID) {
    return (
      ...
      "רוחב 200 ס\"מ תואם לאיירפלור, עובי 30 ס\"מ לבלימה מקסימלית, אורך 2.5 מ'."
    );
  }
  return (
    `מזרן נחיתה מקצועי ${formatLandingMatDimensions(v)} מחומר שמשונית עבה — ` +
    `ספיגת זעזועים, בטיחות ועמידות. מחיר: ${v.price.toLocaleString("he-IL")} ₪.`
  );
}
...
const LANDING_MAT_FEATURES: ProductFeature[] = [
  ...
  {
    title: "פתחי אוורור לאימון בטוח",
    description: "פתחי אוורור בדפנות המזרן מאפשרים יציאת אוויר מהירה בעת נחיתה — ספיגה אחידה ויציבה.",
  },
  {
    title: "5 מידות לפי הצורך",
    description: "מגוון גדלים — ממזרן קומפקטי ועד משטח נחיתה רחב — כדי להתאים לחדר, לג׳ימבורי או לאולם אימונים.",
  },
];

const LANDING_MAT_AIRFLOOR_FEATURES: ProductFeature[] = [
  ...LANDING_MAT_FEATURES.filter((f) => f.title !== "5 מידות לפי הצורך"),
  {
  ...
  const base: Partial<Product> = {
    ...
    introParagraphs: isAirfloor
      ? [
          ...
          "עובי 30 ס\"מ ואורך 2.5 מטר מבטיחים משטח נחיתה ובלימה אופטימליים — לעבודה בטוחה, מדויקת ומקצועית.",
        ]
      : [
          `מזרן נחיתה LEVITATE בגודל ${dimsSlash} — מחומר שמשונית עבה, לספיגת זעזועים מקסימלית ובטיחות בכל נחיתה.`,
          "מזרני הנחיתה שלנו תוכננו לאימוני ג׳ימבורי, אקרובטיקה, נינג'ה וקפיצה לגובה — עם גימור מקצועי, עמידות לאורך זמן ופתחי אוורור לביצועים יציבים.",
          "5 גדלים שונים לפי הצורך — בחרו את המידה המתאימה לכם בסרגל המידות למעלה, או הזמינו ישירות את הגודל הזה.",
        ],
